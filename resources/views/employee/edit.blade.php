<x-layout>
    ...
    <div class="hidden sm:block bg-white/10 max-w-lg mx-auto mt-16 rounded-xs relative">
        <form action="/employees/{{ $employee->id }}" method="POST" class="text-left mx-auto w-md">
            @csrf
            @method('PATCH')
            <h1 class="py-8 ml-2 w-md mx-auto text-3xl font-bold">Edit Employee</h1>

            <div class="flex flex-row pt-4">
                <div class="flex flex-col px-2">
                    <label for="first_name" class="font-bold">First Name<span class="text-red-500 ml-2">*</span></label>
                    <input type="text" id="first_name" name="first_name" placeholder="{{ $employee->first_name }}" class="bg-white/15 mx-auto mt-2 rounded-xs p-3">
                    <x-errors name="first_name" />
                </div>
                <div class="flex flex-col px-2">
                    <label for="last_name" class="font-bold">Last Name<span class="text-red-500 ml-2">*</span></label>
                    <input type="text" id="last_name" name="last_name" placeholder="{{ $employee->last_name }}" class="bg-white/15 mx-auto mt-2 rounded-xs p-3">
                    <x-errors name="last_name" />
                </div>
            </div>
            <div class="flex flex-row border-b-2 border-b-white/10 pb-10">
                <div class="flex flex-col px-2">
                    ...
                    <input type="text" id="personal_email" name="personal_email" placeholder="{{ $employee->personal_email }}" class="bg-white/15 mx-auto mt-2 rounded-xs p-3">
                    <x-errors name="personal_email" />
                </div>
                <div class="flex flex-col px-2">
                    <label for="phone" class="mt-6 font-bold">Phone<span class="text-red-500 ml-2">*</span></label>
                    <input type="text" id="phone" name="phone" placeholder="{{ $employee->phone }}" class="bg-white/15 mx-auto mt-2 rounded-xs p-3">
                    <x-errors name="phone" />
                </div>
            </div>
    ...
    <div class="text-center mt-8 text-lg">
        <a href="/employees/{{ $employee->id }}" class="hover:underline"">Return to Employee</a>
    </div>
</x-layout>